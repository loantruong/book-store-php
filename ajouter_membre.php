...
<?php
    if(isset($_POST["envoyer"])){
        ...
        //Vérifier si login déjà existant
            ...
            //si erreur (1= existe)
            if($membre[0] == 1){
                header("location:ajouter_membre.php?erreurLogin&prenom=$prenom&nom=$nom&mail=$mail");
                exit();
            }
        
        //Vérifier si mdp1 est identique à mdp2
        if($mdp1 != $mdp2){
            header("location:ajouter_membre.php?erreurMdp&prenom=$prenom&nom=$nom&mail=$mail&login=$login");
            exit();
        }
        ...
        if($_FILES["avatar"]["error"] == 0){
            ...
            if(in_array($ext, $extensions)){
                ...
                if(($taille[0] <= 500) && ($taille[1] <= 500)){
                    ...
                    if(!$deplacer){
                        header("location:ajouter_membre.php?erreurDeplacer&prenom=$prenom&nom=$nom&mail=$mail&login=$login");
                        exit();
                    }
                    
                }else{
                    header("location:ajouter_membre.php?erreurTaille&prenom=$prenom&nom=$nom&mail=$mail&login=$login");
                    exit();
                }
            }else{
                header("location:ajouter_membre.php?erreurType&prenom=$prenom&nom=$nom&mail=$mail&login=$login");
                exit();
            }
        }elseif(($_FILES["avatar"]["error"] == 1) || ($_FILES["avatar"]["error"] == 2)){
            //si image trop lourde
            header("location:ajouter_membre.php?erreurLourd&prenom=$prenom&nom=$nom&mail=$mail&login=$login");
            exit();
        }
        ...
        if(!exec){
            ...
            header("location:ajouter_membre.php?erreurRequete&prenom=$prenom&nom=$nom&mail=$mail&login=$login");
            exit();
        }else{
            //connecté la personne pour ne pas à avoir à entrer de nouveau ses id
            $_SESSION["login"] = $login;
            $_SESSION["role"] = "Membre";
            header("location:admin_livres.php?nouveauMembre");
            exit();
        }
