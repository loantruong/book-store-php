...
<form method="post" action="" enctype="multipart/form-data">
    ...
    <div>
        ...
        <a href="delete_avatar">Supprimer Avatar</a>
    </div>
...
<?php
    if(isset($_POST["envoyer"])){
        ...
        if($_FILES["avatar"]["error"] == 0){
            ...
            if(in_array($ext, $extensions)){
                ...
                if(($taille[0] <= 500) && ($taille[1] <= 500)){
                    ...
                    if(!$deplacer){
                        header("location:add_member.php?erreurDeplacer&prenom=$prenom&nom=$nom&mail=$mail");
                        exit();
                    }
                    
                }else{
                    header("location:add_member.php?erreurTaille&prenom=$prenom&nom=$nom&mail=$mail");
                    exit();
                }
            }else{
                header("location:add_member.php?erreurType&prenom=$prenom&nom=$nom&mail=$mail");
                exit();
            }
        }elseif(($_FILES["avatar"]["error"] == 1) || ($_FILES["avatar"]["error"] == 2)){
            //si image trop lourde
            header("location:add_member.php?erreurLourd&prenom=$prenom&nom=$nom&mail=$mail");
            exit();
        }
        ...
        if(!exec){
            ...
            header("location:update_member.php?erreurRequete&prenom=$prenom&nom=$nom&mail=$mail");
            exit();
        }else{
            
            header("location:admin_books.php?modifierMembre");
            exit();
        }
